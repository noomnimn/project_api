...
//-----------------------------------------------------------------ครุภัณฑ์ต่ำวาเกณ์ นอกแผน
exports.findAll2567_LowOut = (req, res, next) => {
    req.getConnection((err, connection) => {
        if (err) return next(err)
        var sql = "select * from 2567_lower_out";
        // var params = "%" + req.query.term + "%"
        connection.query(sql, (err, results) => {
            if (err) return next(err)
            res.send(results)
...
exports.findId2567_LowOut = (req, res, next) => {
    var id = parseInt(req.params.id)
    req.getConnection((err, connection) => {
        if (err) return next(err)
        var sql = "select * from 2567_lower_out where id = ? ";
        connection.query(sql, [id], (err, results) => {
            if (err) return next(err)
            res.send(results[0])
        })
...
exports.updateOutLow_2567 = (req, res, next) => {
    var id = parseInt(req.params.id)
    var { body } = req
    var post = {
        code: body.code ? body.code : null,
    ...
    req.getConnection(function (err, connection) {
        connection.query("update 2567_lower_out set ? where id =?", [post, id], function (err, results) {
            if (err) return next(err)
            res.send({ status: 'ok', results })
        })
    })
...
exports.addOutLow_2567 = (req, res, next) => {
    var { body } = req
    var post = {
        id: body.id,
        code: body.code ? body.code : null,
   ...
   // post.length = post.length.join(',');
    // post.reply_date = post.reply_date.join('-');
    req.getConnection(function (err, connection) {
        connection.query("SELECT code FROM 2567_lower_out  WHERE code= ?", [post.code], function (err, results) {
            if (err) return next(err)
            if (results.length > 0) {
                res.send({
                    status: 201, message: 'เคยลงแล้วค่ะ'
                ...
                connection.query("insert into 2567_lower_out set ?", post, (err, results) => {
                    if (err) return next(err)
                    res.send({ status: 'ok', results })
                })
            }
...
//---------------------------------------------------------------นอกแผน งบลงทุน 2567
exports.findAllOut2567 = (req, res, next) => {
    req.getConnection((err, connection) => {
        if (err) return next(err)
        var sql = "select * from budget_out_2567";
        // var params = "%" + req.query.term + "%"
        connection.query(sql, (err, results) => {
            if (err) return next(err)
            res.send(results)
...
exports.findById2567 = (req, res, next) => {
    var id = parseInt(req.params.id)
    req.getConnection((err, connection) => {
        if (err) return next(err)
        var sql = "select * from budget_out_2567 where id = ? ";
        connection.query(sql, [id], (err, results) => {
            if (err) return next(err)
            res.send(results[0])
        })
...
exports.updateOut2567 = (req, res, next) => {
    var id = parseInt(req.params.id)
    var { body } = req
    var post = {
        code: body.code ? body.code : null,
...
exports.addOut2567 = (req, res, next) => {
    var { body } = req
    var post = {
        id: body.id,
        code: body.code ? body.code : null,
   ...
   // post.length = post.length.join(',');
    // post.reply_date = post.reply_date.join('-');
    req.getConnection(function (err, connection) {
        connection.query("SELECT code FROM budget_out_2567  WHERE code= ?", [post.code], function (err, results) {
            if (err) return next(err)
            if (results.length > 0) {
                res.send({
                    status: 201, message: 'เคยลงแล้วค่ะ'
